...
export const api = {
  ...
  uploadAdminPhoto: (formData) => {
    const token = getAuthToken();
    const headers = {};
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }
    return fetch('/api/admin/photos/upload', {
      method: 'POST',
      credentials: 'include',
      headers,
      body: formData
  ...
  listAdminPhotos: (options = {}) => {
    ...
    return request(`/admin/photos/list${params.toString() ? '?' + params.toString() : ''}`);
  },
  getAdminPhoto: (photoId) => request(`/admin/photos/${photoId}`),
  updateAdminPhoto: (photoId, data) => request(`/admin/photos/${photoId}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteAdminPhoto: (photoId) => request(`/admin/photos/${photoId}`, { method: 'DELETE' }),
  getPhotoStats: () => request('/admin/photos/stats'),
  batchDeletePhotos: (photoIds) => request('/admin/photos/batch/delete', { method: 'POST', body: JSON.stringify({ photoIds }) }),

  // Photo management - User
  uploadPhotoForPost: (formData) => {
    const token = getAuthToken();
    const headers = {};
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
  ...
  getUserPhotoStats: () => request('/photos/stats'),
  getPostPhotos: (postId) => request(`/posts/${postId}/photos`),
  updatePostPhoto: (postId, photoId, data) => request(`/posts/${postId}/photos/${photoId}`, { method: 'PUT', body: JSON.stringify(data) }),
  deletePostPhoto: (postId, photoId) => request(`/posts/${postId}/photos/${photoId}`, { method: 'DELETE' }),
  reorderPostPhotos: (postId, photoIds) => request(`/posts/${postId}/photos/reorder`, { method: 'PUT', body: JSON.stringify({ photoIds }) }),
};

// Helper functions for photo operations (not in api object)
...
export function deletePhoto(photoId) {
  return request(`/admin/photos/${photoId}`, { method: 'DELETE' });
}

export function updatePhoto(photoId, data) {
  return request(`/admin/photos/${photoId}`, { method: 'PUT', body: JSON.stringify(data) });
}

export function getPhotoStats() {
  return request('/admin/photos/stats');
}
