...
Cuando la app inicia, se crea la infraestructura:
...
meru_payment_links (
  ...
  used_by VARCHAR(255),              -- ID del usuario que lo usó
  used_by_username VARCHAR(255),     -- Username de quien lo activó
...
Se inserta una lista predefinida de 10 links Meru:
...
// Para cada link:
await meruLinkService.addLink(link.code, link.url, 'lifetime-pass');
```

**Resultado:** Tabla con 10 links, todos con `status = 'active'`

---

## **PASO 2️⃣: USUARIO INICIA ACTIVACIÓN**
**Archivo:** `src/bot/handlers/user/onboarding.js` (línea 142)
...
Para activar tu Lifetime Pass, por favor presiona el botón de abajo
...
- Flag `waitingForLifetimeCode = true` (línea 85)
- Espera a que usuario envíe el código

---

## **PASO 3️⃣: USUARIO ENVÍA EL CÓDIGO**
**Archivo:** `src/bot/handlers/user/onboarding.js` (línea 429-468)
...
```javascript
const rawCode = ctx.message?.text?.trim();  // "LSJUek"

// Validación 1: No vacío, sin espacios
if (!rawCode || rawCode.length === 0 || rawCode.includes(' ')) {
  await ctx.reply('❌ Formato de código inválido');
  return;
}

// Validación 2: Verificar que el código existe en lifetime-pass.html
const htmlContent = await fs.readFile('/path/to/lifetime-pass.html', 'utf8');
...
while ((match = meruLinksRegex.exec(htmlContent)) !== null) {
    meruCodes.push(match[1]);  // ["LSJUek", "FCqG-z", ...]
}
...
if (!matchingLinkCode) {
  await ctx.reply('❌ Código no encontrado o inválido');
  return;
}
```
...
**Archivo:** `src/services/meruPaymentService.js` (línea 43-130)
...
const paymentCheck = await meruPaymentService.verifyPayment(
...
Si el usuario **PAGÓ en Meru**, la página muestra uno de estos mensajes:
...
const isPaid = paidPatterns.some(
  pattern => pageContent.includes(pattern) || visibleText.includes(pattern)
);
```
...
**Archivo:** `src/bot/handlers/user/onboarding.js` (línea 483-550)

### **5.1 Si `isPaid === true`:**

```javascript
if (paymentCheck.isPaid) {
  const userId = ctx.from.id;              // ID de Telegram
  ...
  if (!activated) {
    await ctx.reply('❌ Ocurrió un error durante la activación');
    return;
  }
}
```
...
SET status = 'used',
    used_by = $2,                      -- ID del usuario
    used_by_username = $3,             -- Username
    used_at = NOW()
...
**Archivo:** `src/bot/handlers/user/onboarding.js` (línea 521-544)

```javascript
await PaymentHistoryService.recordPayment({
  userId: 123456789,
  paymentMethod: 'meru',                 // Método: Meru
  amount: 50,                            // Precio: $50 USD
  currency: 'USD',
  planId: 'lifetime_pass',
  planName: 'Lifetime Pass',
  product: 'lifetime-pass',
  paymentReference: 'LSJUek',            // Código del link
...
metadata          | {"meru_link": "...", "verification_method": "puppeteer"}
```

---

## **PASO 7️⃣: NOTIFICACIONES FINALES**
**Archivo:** `src/bot/handlers/user/onboarding.js` (línea 546-550)
...
```
STATUS = 'active'  → Link disponible para usar
        ↓
User envía código
        ↓
...
isPaid = true
        ↓
STATUS = 'used'    → Link marcado como usado, no puede reutilizarse
```
...
1. **En BD:** Una vez `status = 'used'`, nunca un usuario puede usarlo de nuevo
2. **En lógica:** Solo se procesan links con `status = 'active'`
3. **En Meru:** El mismo link de pago no se puede pagar 2 veces
