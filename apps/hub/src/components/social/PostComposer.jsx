...
export default function PostComposer({ onPosted, onError }) {
  ...
  const handleMediaSelect = (e) => {
    ...
    if (!isImage) {
      const errorMsg = 'Los videos están deshabilitados. Por favor usa YouTube o Vimeo y comparte el enlace.';
      setError(errorMsg);
    ...
    if (file.size > 10 * 1024 * 1024) {
      const errorMsg = 'El archivo es demasiado grande (máximo 10MB). Por favor comprime la imagen.';
      setError(errorMsg);
      showToast(errorMsg, 'warning');
      return;
    }
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Error al publicar. Por favor intenta de nuevo.';
