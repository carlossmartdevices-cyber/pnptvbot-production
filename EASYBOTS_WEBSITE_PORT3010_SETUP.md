# 🚀 EasyBots Website - Port 3010 Deployment Guide

## Status: ✅ CONFIGURED & READY

**Configuration Date:** November 23, 2025  
**Port:** 3010  
**URL:** https://easybots.store/easybots  
**Direct Access:** https://easybots.store (with routing rules)
...
- SSL/HTTPS automatic
- Access logs: `/var/log/nginx/easybots-website-access.log`
...
sudo cp /root/pnptvbot-production/nginx-config-updated.conf /etc/nginx/sites-available/easybots.store
...
### Test Nginx Routing
```bash
curl https://easybots.store/easybots
...
# 15  pnptv-bot            online
# 1   social-hub           online
# ?   easybots-website     online  ← New process
```
...
# Access logs
tail -f /var/log/nginx/easybots-website-access.log

# Error logs
tail -f /root/.pm2/logs/easybots-website-error.log
```
...
### Public URLs (HTTPS)
1. `https://easybots.store/easybots` → Routes to http://localhost:3010
2. `https://easybots.store/easybots/` → Same as above
3. `https://easybots.store/easybots/api/*` → API routes on port 3010
...
| Task | Command |
|------|---------|
| Start website | `pm2 start ecosystem.config.js --name easybots-website` |
| Stop website | `pm2 stop easybots-website` |
| Restart website | `pm2 restart easybots-website` |
| View logs | `pm2 logs easybots-website` |
| Check status | `pm2 status` |
| Direct access | `curl http://localhost:3010` |
| Public URL | `https://easybots.store/easybots` |
