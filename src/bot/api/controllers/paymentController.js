...
class PaymentController {
  ...
  static async getPaymentInfo(req, res) {
    try {
      ...
      if (provider === 'epayco') {
        ...
        basePaymentData.paypalClientId = process.env.PAYPAL_CLIENT_ID;
  ...
  static async createPayPalOrder(req, res) {
    try {
      ...
      if (!payment) {
        logger.warn('Payment not found for PayPal order creation', { paymentId });
        return res.status(404).json({
          success: false,
      ...
      if (!plan) {
        logger.error('Plan not found for PayPal order', { paymentId, planId });
        return res.status(404).json({
          success: false,
      ...
      // Create PayPal order
      const webhookDomain = process.env.BOT_WEBHOOK_DOMAIN;
      const returnUrl = `${webhookDomain}/api/payment-response?status=success&payment_id=${paymentId}`;
      const cancelUrl = `${webhookDomain}/api/payment-response?status=cancelled&payment_id=${paymentId}`;
      ...
      logger.info('PayPal order created via API', {
        paymentId,
  ...
  static async capturePayPalOrder(req, res) {
    try {
      ...
      if (!payment) {
        logger.warn('Payment not found for PayPal capture', { paymentId });
        return res.status(404).json({
          success: false,
          error: 'Payment not found',
        });
      ...
      try {
        if (userId && planId) {
          ...
          if (plan) {
            ...
            logger.info('Subscription activated after PayPal capture', {
              userId,
              planId,
              paymentId,
              expiryDate,
            ...
            const amount = captureResult.amount || 0;
        ...
        logger.error('Error activating subscription after PayPal capture', {
      ...
      logger.info('PayPal order captured via API', {
        paymentId,
        orderId: orderID,
