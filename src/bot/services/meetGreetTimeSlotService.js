...
/**
 * Meet & Greet Time Slot Service
 * Generates time slots with specific constraints:
 * - Duration: 20 hours to 200 hours (next day)
 * - Days: Thursday to Monday
 * - Buffer: 15 minutes between slots
 */

class MeetGreetTimeSlotService {
  /**
   * Generate time slots for a model
  ...
  static async generateTimeSlots(modelId, startDate, durationMinutes) {
    try {
      ...
      for (let dayOffset = 0; dayOffset < daysToGenerate; dayOffset++) {
        ...
        // We want slots with duration from 20 hours to 200 hours (next day)
        // This means we generate slots that can accommodate calls of varying lengths
        // within a 20-hour to 200-hour window

        // For simplicity, let's generate slots in 20-hour blocks
        // with 15-minute buffers between them
        const startHour = 10; // Start at 10 AM
        const endHour = 22;   // End at 10 PM (20 hours later)

        // Generate slots every 15 minutes + duration + 15 minute buffer
      ...
      logger.info('Generated Meet & Greet time slots', {
        modelId,
        count: slots.length,
        durationMinutes,
      ...
      logger.error('Error generating Meet & Greet time slots:', error);
      throw new Error('Failed to generate time slots: ' + error.message);
    }
  }

  /**
   * Generate extended time slots (20-200 hours)
   * @param {number} modelId - Model ID
   * @param {Date} startDate - Start date for slot generation
  ...
  static async generateExtendedTimeSlots(modelId, startDate) {
    try {
      ...
      logger.error('Error generating extended Meet & Greet time slots:', error);
  ...
  static getNextMondayAfterThursday() {
    ...
    nextMonday.setDate(nextThursday.getDate() + 5); // Thursday + 5 days = Tuesday, wait no...
    // Thursday (4) + 4 days = Monday (1, since 4+4=8, 8%7=1)
    nextMonday.setDate(nextThursday.getDate() + 4);
...
module.exports = MeetGreetTimeSlotService;