...
const registerMembersAreaHandlers = (bot) => {
    ...
    bot.action('show_members_area', async (ctx) => {
        try {
            ...
            // Build video rooms URL with user context for JaaS
            const displayName = ctx.from.first_name || 'Guest';
            const videoRoomsUrl = `https://pnptv.app/video-rooms?userId=${userId}&displayName=${encodeURIComponent(displayName)}`;
