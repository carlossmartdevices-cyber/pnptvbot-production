-- Migration: 002_role_system_complete.sql
-- Created: 2026-02-19
-- Description: Complete role-based access control system (fixed version)
-- NOTE: Separated role creation from foreign key setup
...
-- Create audit_logs table (with VARCHAR for user IDs)
...
-- Insert default roles BEFORE adding foreign keys to users
...
-- NOW add role columns to users
ALTER TABLE users ADD COLUMN IF NOT EXISTS role_id INTEGER DEFAULT 5 REFERENCES roles(id);
